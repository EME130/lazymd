...
test "highlight zig keywords" {
    ...
    try std.testing.expectEqual(TokenKind.keyword, spans.items[0].kind);
...
test "highlight line comment" {
    ...
    try std.testing.expectEqual(TokenKind.comment, spans.items[0].kind);
}
...
test "highlight zig builtin" {
    ...
    try std.testing.expect(spans.items.len > 0);
    try std.testing.expectEqual(TokenKind.builtin, spans.items[0].kind);
}
